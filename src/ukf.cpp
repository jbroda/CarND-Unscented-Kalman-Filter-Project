...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) 
{
	...
	*/
	...
	if (! is_initialized_)
	{
		...
		// Set the covariance matrix to identity.
		this->P_.setIdentity();
	...
	double dt = (meas_package.timestamp_ - this->time_us_) / 1000000.0;	//dt - expressed in seconds
	this->time_us_ = meas_package.timestamp_;
...
void UKF::Prediction(double delta_t) 
{
	...
	*/
	...
	// Computed the predicted state covariance matrix.
...
void UKF::UpdateLidar(MeasurementPackage meas_package) 
{
	...
	*/
	...
	for (int i = 0; i < 2 * n_aug_ + 1; i++) {
		...
		// angle normalization
		while (x_diff(3) > M_PI) z_diff(3) -= 2.*M_PI;
		while (x_diff(3) < -M_PI) z_diff(3) += 2.*M_PI;
